    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        ...
        <div class="space-y-4">
            ...
            <div x-data="{ open: false }" class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <button @click="open = !open" type="button" class="w-full px-6 py-4 bg-slate-50 hover:bg-slate-100 transition-colors flex justify-between items-center outline-none">
                    <h4 class="font-bold text-slate-700 flex items-center gap-3">
                        <span class="w-8 h-8 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center text-sm">2</span> Unit Kerja
                    </h4>
                    <svg :class="{'rotate-180': open}" class="w-5 h-5 text-slate-500 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                </button>
                <div x-show="open" x-transition style="display: none;" class="p-6 border-t border-slate-200">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-y-5 gap-x-6 text-sm">
                        <div><span class="block text-slate-400 text-xs mb-1">Unit Kerja</span><p class="font-semibold text-slate-800">{{ $industri?->unit_kerja ?? '-' }}</p></div>
                        <div><span class="block text-slate-400 text-xs mb-1">Jabatan</span><p class="font-semibold text-slate-800">{{ $industri?->jabatan ?? '-' }}</p></div>
                        <div><span class="block text-slate-400 text-xs mb-1">Golongan</span><p class="font-semibold text-slate-800">{{ $industri?->golongan ?? '-' }}</p></div>
                        ...
                        <div><span class="block text-slate-400 text-xs mb-1">Tanggal Pensiun</span><p class="font-semibold text-slate-800">{{ $industri?->tanggal_pensiun ?? '-' }}</p></div>
            ...
            <div x-data="{ open: false }" class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div x-show="open" x-transition style="display: none;" class="p-6 border-t border-slate-200">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-5 gap-x-6 text-sm">
                        ...
                        <div><span class="block text-slate-400 text-xs mb-1">Lokasi Kampus (Kota)</span><p class="font-semibold text-slate-800">{{ $universitas?->kota ?? '-' }}</p></div>
            ...
            <div x-data="{ open: false }" class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <button @click="open = !open" type="button" class="w-full px-6 py-4 bg-slate-50 hover:bg-slate-100 transition-colors flex justify-between items-center outline-none">
                    <h4 class="font-bold text-slate-700 flex items-center gap-3">
                        <span class="w-8 h-8 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center text-sm">4</span> Surat Rekomendasi
                    </h4>
                    <svg :class="{'rotate-180': open}" class="w-5 h-5 text-slate-500 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                </button>
                <div x-show="open" x-transition style="display: none;" class="p-6 border-t border-slate-200">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-y-5 gap-x-6 text-sm">
                        <div><span class="block text-slate-400 text-xs mb-1">Judul (Kategori)</span><p class="font-semibold text-slate-800">{{ $rekomendasi?->kategori ?? '-' }}</p></div>
            ...
            <div x-data="{ open: false }" class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <button @click="open = !open" type="button" class="w-full px-6 py-4 bg-slate-50 hover:bg-slate-100 transition-colors flex justify-between items-center outline-none">
                    <h4 class="font-bold text-slate-700 flex items-center gap-3">
                        <span class="w-8 h-8 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center text-sm">5</span> Essay Kontribusi
                    </h4>
                    <svg :class="{'rotate-180': open}" class="w-5 h-5 text-slate-500 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                </button>
                <div x-show="open" x-transition style="display: none;" class="p-6 border-t border-slate-200">
            ...
            <div x-data="{ open: false }" class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <button @click="open = !open" type="button" class="w-full px-6 py-4 bg-slate-50 hover:bg-slate-100 transition-colors flex justify-between items-center outline-none">
                    <h4 class="font-bold text-slate-700 flex items-center gap-3">
                        <span class="w-8 h-8 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center text-sm">6</span> Surat Komitmen
                    </h4>
                    <svg :class="{'rotate-180': open}" class="w-5 h-5 text-slate-500 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                </button>
                <div x-show="open" x-transition style="display: none;" class="p-6 border-t border-slate-200">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-5 gap-x-6 text-sm">
                        <div class="md:col-span-2">
                            <span class="block text-slate-400 text-xs mb-1">File Surat Komitmen (PDF)</span>
                            ...
                            @else 
                                <span class="text-red-500 text-xs font-semibold">Belum Diunggah</span> 
                            @endif
                        </div>
                    </div>
                    <div class="mt-4">
