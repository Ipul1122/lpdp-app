...
<aside class="w-20 bg-white border-r border-slate-200 h-screen fixed left-0 top-0 flex flex-col items-center py-6 z-50 transition-transform duration-300 ease-in-out md:translate-x-0"
    ...
    <nav class="flex flex-col space-y-4 w-full px-3">
        ...
        <a href="{{ route('pendaftaran.index') }}" 
           class="w-full aspect-square rounded-2xl flex items-center justify-center transition-all duration-200 
           {{ Request::is('pendaftaran*') ? 'bg-orange-500 text-white shadow-md shadow-orange-200' : 'text-slate-400 hover:bg-orange-50 hover:text-orange-500' }}"
           title="Pendaftaran">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="{{ route('riwayat.index') }}" 
           class="w-full aspect-square rounded-2xl flex items-center justify-center transition-all duration-200 
           {{ Request::is('riwayat*') ? 'bg-orange-500 text-white shadow-md shadow-orange-200' : 'text-slate-400 hover:bg-orange-50 hover:text-orange-500' }}"
           title="Riwayat">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="{{ route('notifikasi.index') }}" 
           class="w-full aspect-square rounded-2xl flex items-center justify-center transition-all duration-200 relative
           {{ Request::is('notifikasi*') ? 'bg-orange-500 text-white shadow-md shadow-orange-200' : 'text-slate-400 hover:bg-orange-50 hover:text-orange-500' }}"
           title="Notifikasi">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
            </svg>
            @php
                $sidebarUnreadCount = Auth::check() ? \App\Models\Notification::where('user_id', Auth::id())->where('is_read', false)->count() : 0;
            @endphp
